...
export function PageLoadingState({
  ...
  return (
    <div
      className={cn(
        'flex min-h-[280px] w-full flex-col items-center justify-center rounded-2xl border border-stroke-soft-200 bg-bg-white-0 p-8 shadow-regular-xs transition-all duration-300',
        className,
      )}
      role="status"
      aria-live="polite"
    ...
    >
      ...
      {subMessage && (
        <p className="mt-1 text-paragraph-xs text-text-sub-600">{subMessage}</p>
      )}
    </div>
  );
}
